...
public:
    ...
    void dfs(int row,int col,vector<vector<int>>& image,vector<vector<int>>& ans,int iniColor,int newColor,int n,int m){
        ...
        dfs(row,col-1,image,ans,iniColor,newColor,n,m);//left
        dfs(row-1,col,image,ans,iniColor,newColor,n,m);//up
        dfs(row,col+1,image,ans,iniColor,newColor,n,m);//right
        dfs(row+1,col,image,ans,iniColor,newColor,n,m);//down
    }
